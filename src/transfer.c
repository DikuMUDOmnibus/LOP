...
TRANSFER_DATA *imc_in_transfer;
TRANSFER_DATA *imc_out_transfer;
...
void save_transfer( void )
{
   ...
   fprintf( fp, "MCCPOut   %d %d %d %d\n", mccp_out_transfer->giga, mccp_out_transfer->mega, mccp_out_transfer->kilo, mccp_out_transfer->byte );
   fprintf( fp, "MCCPSaved %d %d %d %d\n", mccp_saved_transfer->giga, mccp_saved_transfer->mega, mccp_saved_transfer->kilo, mccp_saved_transfer->byte );
   fprintf( fp, "IMCIn     %d %d %d %d\n", imc_in_transfer->giga, imc_in_transfer->mega, imc_in_transfer->kilo, imc_in_transfer->byte );
   fprintf( fp, "IMCOut    %d %d %d %d\n", imc_out_transfer->giga, imc_out_transfer->mega, imc_out_transfer->kilo, imc_out_transfer->byte );
...
void update_transfer( int type, int size )
{
   ...
   else if( type == 5 )
   {
      if( imc_in_transfer )
      {
         imc_in_transfer->byte += size;
         update_bytes( imc_in_transfer );
      }
   }
   else if( type == 6 )
   {
      if( imc_out_transfer )
      {
         imc_out_transfer->byte += size;
         update_bytes( imc_out_transfer );
      }
   }
   ...
   if( type == 1 || type == 5 )
   {
      ...
      {
         total_in_transfer->giga = ( in_transfer->giga + imc_in_transfer->giga );
         total_in_transfer->mega = ( in_transfer->mega + imc_in_transfer->mega );
         total_in_transfer->kilo = ( in_transfer->kilo + imc_in_transfer->kilo );
         total_in_transfer->byte = ( in_transfer->byte + imc_in_transfer->byte );
   ...
   else if( type == 2 || type == 3 || type == 6 )
   {
      ...
      {
         total_out_transfer->giga = ( out_transfer->giga + mccp_out_transfer->giga + imc_out_transfer->giga );
         total_out_transfer->mega = ( out_transfer->mega + mccp_out_transfer->mega + imc_out_transfer->mega );
         total_out_transfer->kilo = ( out_transfer->kilo + mccp_out_transfer->kilo + imc_out_transfer->kilo );
         total_out_transfer->byte = ( out_transfer->byte + mccp_out_transfer->byte + imc_out_transfer->byte );
...
void clear_transfer( void )
{
   ...
   if( imc_in_transfer )
      clear_bytes( imc_in_transfer );
   if( imc_out_transfer )
      clear_bytes( imc_out_transfer );
   if( total_in_transfer )
...
void create_transfer( void )
{
   ...
   if( !imc_in_transfer )
      CREATE( imc_in_transfer, TRANSFER_DATA, 1 );
   if( !imc_out_transfer )
      CREATE( imc_out_transfer, TRANSFER_DATA, 1 );
   if( !total_in_transfer )
...
void free_transfer( void )
{
   ...
   if( imc_in_transfer )
      DISPOSE( imc_in_transfer );
   if( imc_out_transfer )
      DISPOSE( imc_out_transfer );
   if( total_in_transfer )
...
void show_transfer( CHAR_DATA *ch, TRANSFER_DATA *transfer )
{
   ...
   if( transfer->byte > 0 )
      ch_printf( ch, "%s%dB", ( transfer->giga > 0 || transfer->mega > 0 || transfer->kilo > 0 ) ? " " : "", transfer->byte );
}

CMDF( do_check_transfer )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      send_to_char( "\r\n&wIMC data received  :  &C", ch );
      show_transfer( ch, imc_in_transfer );
      send_to_char( "\r\n&wIMC data sent      :  &C", ch );
      show_transfer( ch, imc_out_transfer );
   ...
   for( ;; )
   {
      ...
      switch( UPPER( word[0] ) )
      {
         ...
         case 'I':
            ...
            if( !str_cmp( word, "IMCIn" ) )
            {
               imc_in_transfer->giga = fread_number( fp );
               imc_in_transfer->mega = fread_number( fp );
               imc_in_transfer->kilo = fread_number( fp );
               imc_in_transfer->byte = fread_number( fp );
               fMatch = true;
               break;
            }
            if( !str_cmp( word, "IMCOut" ) )
            {
               imc_out_transfer->giga = fread_number( fp );
               imc_out_transfer->mega = fread_number( fp );
               imc_out_transfer->kilo = fread_number( fp );
               imc_out_transfer->byte = fread_number( fp );
               fMatch = true;
               break;
            }
            break;
