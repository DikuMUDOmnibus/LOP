...
bool can_read( CHAR_DATA *ch, BOARD_DATA *board )
{
   ...
   if( board->read_group )
   {
      ...
      if( ch->pcdata->nation && !str_cmp( ch->pcdata->nation->name, board->read_group ) )
         return true;
      if( ch->pcdata->deity && !str_cmp( ch->pcdata->deity->name, board->read_group ) )
         return true;
   }
...
bool is_note_to( CHAR_DATA *ch, NOTE_DATA *pnote )
{
   ...
   if( is_devoted( ch ) && is_name( ch->pcdata->deity->name, pnote->to_list ) )
      return true;
...
void read_note( CHAR_DATA *ch, int count, BOARD_DATA *board, NOTE_DATA *pnote )
{
   ...
   pager_printf( ch, "%s[%s%3d%s] %s%s%s: %s%s\r\n%s\r\n%sTo: %s%s\r\n",
      color1, color2, count, color1, color2, pnote->sender, color1, color2, pnote->subject,
   ...
   if( arg == NULL || arg[0] == '\0' || !str_cmp( arg, "list" ) )
   {
      ...
      for( pnote = board->first_note; pnote; pnote = pnote->next )
      {
         ...
         pager_printf( ch, "%3d> %12.12s %25s",
            count, pnote->sender ? pnote->sender : "(No Sender)", num_punct( pnote->autowin ) );
   ...
   if( is_number( arg ) )
   {
      ...
      for( pnote = board->first_note; pnote; pnote = pnote->next )
      {
         ...
         if( !str_cmp( arg, "bid" ) )
         {
            ...
            to_channel_printf( "auction", PERM_ALL, "%s has bidded %s gold on %s.", ch->name, num_punct( count ), pnote->subject );
            auction_update( );
            write_board( board );
            return;
         }
