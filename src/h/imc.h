...
#define IMCTOGGLE_BIT( var, bit )   ( (var) = !(bit) )
...
{
   IMC_CMD_DATA *next, *prev;
   IMC_ALIAS *first_alias, *last_alias;
   IMC_FUN *function;
   char *name;
   int level;
   bool connected;
...
struct imc_ignore
{
   IMC_IGNORE *next, *prev;
   char *name;
};

struct imcucache_data
{
   IMCUCACHE_DATA *next, *prev;
   char *name;
   time_t time;
   int gender;
};
...
struct imcchar_data
{
   IMC_IGNORE *imcfirst_ignore, *imclast_ignore;
...
struct imc_packet
{
   IMC_PDATA *first_data, *last_data;
   char from[SMST];
   char to[SMST];
   char type[SMST];
...
struct imc_siteinfo
{
   ...
   time_t lttime;          /* Last time something was transfered */
   short kltime;           /* So we can keep the connection alive */
